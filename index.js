'use strict'
// 全局声明插件代号
const pluginname = 'butterfly_categories_card_gabriel'
// 全局声明依赖
...
// =====================================================================
  ...
  if (!(config && config.enable)) return
    ...
    categories_list.forEach((item, index) => {
      // 根据配置的 name 准确匹配
      var cfgItem = categories_message.filter(t => t.name === item.name);
      new_categories_list[index] = Object.assign(item, (1 === cfgItem.length) ? cfgItem[0] : {});
    });
  ...
  // 集体声明配置项
    const data = {
      ...
      custom_css: config.custom_css ? urlFor(config.custom_css) : "https://cdn.jsdelivr.net/npm/hexo-butterfly-categories-card-gabriel/lib/categorybar.css"
    }
  // 渲染页面
